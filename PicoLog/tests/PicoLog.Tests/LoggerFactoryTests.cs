...
public sealed class LoggerFactoryTests
{
    ...
    [Test]
    public async Task Pipeline_DispatcherWakesUp_EvenWhenThreadPoolIsSaturated()
    {
        // Regression test: CategoryPipeline's dedicated processing thread
        // blocks on _queue.WaitToReadAsync().AsTask().GetAwaiter().GetResult().
        // If the underlying Channel is created with
        // AllowSynchronousContinuations = false, every TryWrite needs a
        // ThreadPool worker to run the wakeup continuation that completes
        // the reader's Task. On low-core ARM64 CI runners (win-arm64 and
        // osx-arm64, typically 2-4 cores) running 166 TUnit tests in
        // parallel, the pool can be fully saturated by other tests' work,
        // which starves the wakeup continuations and hangs every test that
        // observes the dispatcher via a sink-side latch (BlockingSink,
        // CoordinatedDisposalSink, etc.) for the full CI timeout (45 min).
        //
        // Contract: an entry written to the pipeline's queue must be visible
        // to the dispatcher thread (and therefore reach the sink) in bounded
        // time regardless of ThreadPool availability.
        //
        // This test reproduces the failure mode by saturating the
        // ThreadPool with blocking work that won't release until the test
        // signals it, then asserting the sink still receives the entry
        // within a short budget. Pre-fix (AllowSync = false): the dispatcher
        // never wakes, the sink never sees the write, the test times out.
        // Post-fix (AllowSync = true): the wakeup runs inline on the test
        // thread that called logger.Info, the dispatcher resumes, and the
        // sink sees the entry promptly.
        var poolSaturationRelease = new ManualResetEventSlim(initialState: false);
        var poolSaturationStarted = new CountdownEvent(0);

        // Saturate the ThreadPool with one blocked work item per available
        // worker so any pending continuations queued on the pool cannot run.
        var workerCount = Math.Max(Environment.ProcessorCount * 2, 8);
        poolSaturationStarted.Reset(workerCount);
        for (var i = 0; i < workerCount; i++)
        {
            ThreadPool.UnsafeQueueUserWorkItem(
                _ =>
                {
                    poolSaturationStarted.Signal();
                    poolSaturationRelease.Wait();
                },
                state: null
            );
        }

        // Wait until the saturation work items have all started running so
        // the pool really is full when we test the wakeup path.
        await Task.Run(() => poolSaturationStarted.Wait(TimeSpan.FromSeconds(5)));

        var sink = new CollectingSink();
        var factory = new LoggerFactory([sink]);
        try
        {
            var logger = factory.CreateLogger("Tests.PoolSaturation");

            // The dispatcher (dedicated thread, NOT a pool worker) must
            // observe this write and append it to the sink. With
            // AllowSynchronousContinuations = true, the WaitToReadAsync
            // wakeup runs inline on this calling thread, so it does not
            // need a free pool worker.
            logger.Info("wake-up");

            // Poll the sink with a generous but bounded budget. 5 seconds
            // is >> what the wakeup needs on any healthy runner; pre-fix
            // this would never see the entry until the pool drains, which
            // never happens.
            var stopwatch = Stopwatch.StartNew();
            while (sink.Entries.Count == 0 && stopwatch.Elapsed < TimeSpan.FromSeconds(5))
            {
                // Use Thread.Sleep (not Task.Delay) because Task.Delay also
                // depends on pool workers (timer queue callback).
                Thread.Sleep(20);
            }
        ...
        {
            // Release the saturation work items first so dispose can use
            // the pool, then dispose the factory.
            poolSaturationRelease.Set();

            // Dispose on a dedicated thread so this test never hangs the
            // host even if the contract is broken in some other way.
            var disposeSignal = new TaskCompletionSource(
                TaskCreationOptions.RunContinuationsAsynchronously
            );
            var disposeThread = new Thread(() =>
            {
                ...
                {
                    // Suppress: primary assertion is what matters.
                }
                finally
                {
                    disposeSignal.TrySetResult();
            ...
            {
                IsBackground = true,
                Name = "PicoLog.Test.PoolSaturationDisposer"
            };
            disposeThread.Start();
            ...
            poolSaturationRelease.Dispose();
            poolSaturationStarted.Dispose();
        }
    }
