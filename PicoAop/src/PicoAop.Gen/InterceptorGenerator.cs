...
public sealed partial class InterceptorGenerator : IIncrementalGenerator
{
    ...
    )
    {
        ...
        )
        {
            ...
            if (isAsyncReturn)
            {
                // InvokeTargetAsync returns ValueTask / ValueTask<T>.
                // If the original method already returns ValueTask/ValueTask<T>,
                // pass through directly (no async state machine).
                // If the original returns Task/Task<T>, use async/await to convert.
