...
// svcRegistry.RegisterSingle<TcpNode>(
//     sp => new TcpNode(tcpOption, sp.Resolve<Func<ITcpHandler>>(), sp.Resolve<ILogger<TcpNode>>())
// );
...
try
{
    ...
    await Task.WhenAny(udpServerTask).GetAwaiter().GetResult();
}
catch (Exception ex)
{
