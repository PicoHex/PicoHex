...
public sealed class InterceptorGenerator : IIncrementalGenerator
{
        ...
        ImmutableArray<InterceptionInfo?> infos)
    {
        ...
        sb.AppendLine($"namespace {PicoAotNames.GeneratedNamespace};");
        sb.AppendLine("using global::PicoAot.Abs;");
        sb.AppendLine();

        var wrappersSb = new StringBuilder();
        wrappersSb.AppendLine("// <auto-generated/>");
        wrappersSb.AppendLine("#nullable enable");
        wrappersSb.AppendLine($"namespace {PicoAotNames.GeneratedNamespace};");
        ...
        foreach (var info in deduped)
        {
            ...
            // Validate
            if (svcType.IsSealed)
            { spc.ReportDiagnostic(Diagnostic.Create(PicoAotDiag.SealedType, Location.None, svcType.Name)); continue; }
            if (svcType.IsValueType)
            { spc.ReportDiagnostic(Diagnostic.Create(PicoAotDiag.ValueType, Location.None, svcType.Name)); continue; }
            ...
            foreach (var method in methods)
            {
                ...
                {
                    spc.ReportDiagnostic(Diagnostic.Create(PicoAotDiag.RefOutMethod,
        ...
        spc.AddSource("PicoAot.Invocations.g.cs", SourceText.From(sb.ToString(), Encoding.UTF8));
        spc.AddSource("PicoAot.Wrappers.g.cs", SourceText.From(wrappersSb.ToString(), Encoding.UTF8));
    }
