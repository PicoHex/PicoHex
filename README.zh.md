...
每个应用程序都要读取配置、组装内部结构并产生输出。**PicoHex** 提供这些操作在 .NET 下的最小化、AOT-first 实现——配置 (PicoCfg)、依赖注入 (PicoDI)、日志 (PicoLog)，以及编译期 AOP (PicoAop) 和中介者派发 (PicoMediator)。全部基于源生成器，零运行时反射。
